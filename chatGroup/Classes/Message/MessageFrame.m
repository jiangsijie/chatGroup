...
//
//        CGSize timeSize = [[MessageHelper getChatTimeWithTime:message.sendTime] boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, kChat_content_maxW) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kChatFont_time} context:nil].size;
//
...
//    }
    ...
    if (!isSpecial) {//特殊消息没有头像
        ...
        _icon = CGRectMake(iconX, content_y, kChat_icon_wh, kChat_icon_wh);
    }
    
    // 计算用户名
    if (_showName){
        CGFloat nameX = kChat_margin + kChat_icon_wh  + kChat_margin + kChat_angle_w;
        _name = CGRectMake(nameX, content_y , kSHWidth - 2*nameX, kChat_name_h);
...
//        {
//            contentSize = [MessageHelper getSizeWithMaxSize:CGSizeMake(kChat_pic_wh, kChat_pic_wh) size:CGSizeMake(message.imageWidth, message.imageHeight)];
//        }
            break;
...
//        {
//            contentSize = [MessageHelper getSizeWithMaxSize:CGSizeMake(kChat_gif_wh, kChat_gif_wh) size:CGSizeMake(self.message.gifWidth, self.message.gifHeight)];
//        }
            break;
    ...
    if (isSpecial) {//特殊消息 居中显示
        ...
        content_x = CGRectGetMaxX(_icon) + kChat_margin;
        if (isSend) {
            content_x = CGRectGetMidX(_icon) - kChat_margin - contentSize.width - 2*kChat_margin;
        }
    }
    
    //聊天气泡frame
    _content = CGRectMake(content_x, content_y, contentSize.width, contentSize.height);
    //cell高度
    _cell_h = CGRectGetMaxY(_content)  + kChat_margin;
}
