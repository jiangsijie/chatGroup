...
#pragma mark 创建内容
- (MessageContentView *)btnContent{
    if (!_btnContent) {
        _btnContent = [MessageContentView buttonWithType:UIButtonTypeCustom];
...
- (ActivityIndicatorView *)activityView{
    if (!_activityView) {
        _activityView = [[ActivityIndicatorView alloc]init];
...
- (void)setMessageFrame:(MessageFrame *)messageFrame {
    ...
    // 设置时间
    self.labelTime.frame = messageFrame.time;
//    self.labelTime.text = [MessageHelper getChatTimeWithTime:message.sendTime];
    
    // 设置头像
    self.btnHeadImage.frame = messageFrame.icon;
    [self.btnHeadImage setBackgroundImage:[FileHelper imageNamed:message.avator] forState:0];
    
    // 设置昵称
    self.labelNum.frame = messageFrame.name;
    ...
    // 设置内容
    self.btnContent.frame = messageFrame.content;
